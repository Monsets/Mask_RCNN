...
def load_image_gt(dataset, config, image_id, augment=False, augmentation=None,
    ...
    if augmentation:
        ...
        depth_map = det.augment_image(depth_map,
                                 hooks=imgaug.HooksImages(activator=hook))
        # Verify that shapes didn't change
        assert image.shape == image_shape, "Augmentation shouldn't change image size"
        assert mask.shape == mask_shape, "Augmentation shouldn't change mask size"
        assert depth_map.shape == depth_map__shape, "Augmentation shouldn't change mask size"
    ...
    # Image meta data
    image_meta = U.compose_image_meta(image_id, original_shape, image.shape,
...
def data_generator(dataset, config, shuffle=True, augment=False, augmentation=None,
    ...
    backbone_shapes = U.compute_backbone_shapes(config, config.IMAGE_SHAPE)
    ...
    while True:
        try:
            ...
            # RPN Targets
            rpn_match, rpn_bbox = U.build_rpn_targets(image.shape, anchors,
                                                    gt_class_ids, gt_boxes, config)

            # Mask R-CNN Targets
            if random_rois:
                rpn_rois = U.generate_random_rois(
                    image.shape, random_rois, gt_class_ids, gt_boxes)
                if detection_targets:
                    rois, mrcnn_class_ids, mrcnn_bbox, mrcnn_mask = \
                        U.build_detection_targets(
